...
class SfdcSession(Session):
    ...
    _LOGIN_TMPL = """<env:Envelope xmlns:xsd='http://www.w3.org/2001/XMLSchema'
...
xmlns:env='http://schemas.xmlsoap.org/soap/envelope/'>
    ...
    def __init__(
        self, username=None, password=None, token=None, is_sandbox=False, api_version=_DEFAULT_API_VERSION, **kwargs
    ):
    ...
    def login(self):
        ...
        if root.find("soapenv:Body/soapenv:Fault", SfdcSession._XML_NAMESPACES):
            raise Exception(
                "Could not log in. Code: %s Message: %s"
                % (
        ...
        self._session_id = root.find(
            "soapenv:Body/d:loginResponse/d:result/d:sessionId", SfdcSession._XML_NAMESPACES
        ).text
